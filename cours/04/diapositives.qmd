...
::: {.notes}
Réserver dix minutes au début de la séance. Les questions évaluées et leur corrigé ne figurent pas dans cette première version : ils seront préparés dans la dernière étape, avec l’événement Wooclap et le fichier d’import. Vérifier la connexion UQAR avant le début.
:::
...
**Que manque le passant pendant cette conversation?**
...
| **Événements** | Séquence habituelle d’une situation | Le scénario d’un restaurant |
...
Un événement perçu comme **lointain** tend à être représenté de façon plus abstraite.

Un événement perçu comme **proche** tend à être représenté avec davantage de détails concrets.
...
Nous pouvons estimer la fréquence ou la probabilité d’un événement à partir de la **facilité avec laquelle des exemples viennent à l’esprit**.
...
- l’impression qu’un événement était prévisible.
